...
import os
import numpy as np
from residual_dense_block import RDB, RDB_32
...
class GridDehazeNetSingle(nn.Module):
    def __init__(self,need_semantic=False, multiscale=False, in_channels=3, depth_rate=16, kernel_size=3, height=1, width=6, num_dense_layer=4, growth_rate=16):
...
class ImageSingleScaleNet(nn.Module):
    def __init__(self, need_semantic=True, multiscale=True, height=1, width=6, num_dense_layer=4, growth_rate=16):
        super(ImageSingleScaleNet, self).__init__()
        self.GDN = GridDehazeNetSingle(need_semantic=True, multiscale=multiscale, height=height, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
        self.need_semantic = need_semantic
        

    def forward(self, x):
        # if need semantic x = [coarse, semantic_attention]
        if self.need_semantic:
            input_pre = x[0]
            x_ori = x[0]
            att = x[1]
        ...
        inp_all = torch.cat((input_pre, x_ori), 1)

        out = self.GDN([inp_all, att])

        return out

class Single_coarse_single_fine(nn.Module):
    def __init__(self, semantic_extract_model=None, in_channels=3, depth_rate=16, kernel_size=3, stride=2, height=1, width=6, num_dense_layer=4, growth_rate=16):
        super(Single_coarse_single_fine, self).__init__()
        self.coarse_model = GridDehazeNetSingle(height=height, multiscale=False, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
        self.level = 1
        self.scale = 1
        self.semantic_extract_model = semantic_extract_model
        self.fine_model = ImageSingleScaleNet(need_semantic=True, multiscale=True, height=height, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
...
class StereoImageNet_noshare(nn.Module):
    def __init__(self, need_semantic=True, multiscale=True, in_channels=3, width=6, depth_rate=16, kernel_size=3, num_dense_layer=4, growth_rate=16):
        super(StereoImageNet_noshare, self).__init__()
        self.width = width
        self.rdb_modules = nn.ModuleDict()
        self.depth_rate = depth_rate
        rdb_in_channels = depth_rate

        self.conv_in = nn.Conv2d(in_channels, depth_rate, kernel_size=kernel_size, padding=(kernel_size - 1) // 2)
        self.conv_in2 = nn.Conv2d(in_channels, depth_rate, kernel_size=kernel_size, padding=(kernel_size - 1) // 2)

        self.rdb_in = RDB(depth_rate, num_dense_layer, growth_rate)
        self.rdb_in2 = RDB(depth_rate, num_dense_layer, growth_rate)

        self.transform_feature_map_to_120 = nn.Conv2d(depth_rate, 120, kernel_size=3, padding=(kernel_size-1) // 2)
        self.transform_feature_map_to_depth_rate = nn.Conv2d(120, depth_rate, kernel_size=3, padding=(kernel_size-1) // 2)

        self.transform_feature_map_to_120_2 = nn.Conv2d(depth_rate, 120, kernel_size=3, padding=(kernel_size-1) // 2)
        self.transform_feature_map_to_depth_rate_2 = nn.Conv2d(120, depth_rate, kernel_size=3, padding=(kernel_size-1) // 2)

        for i in range(2):
            for j in range(self.width-1):
                self.rdb_modules.update({'{}_{}'.format(i,j): RDB_32(rdb_in_channels, num_dense_layer, growth_rate*2)})


        self.rdb_out = RDB_32(rdb_in_channels, num_dense_layer, growth_rate*2)
        self.rdb_out_2 = RDB_32(rdb_in_channels, num_dense_layer, growth_rate*2)
        self.conv_out = nn.Conv2d(depth_rate, in_channels, kernel_size=kernel_size, padding=(kernel_size - 1) // 2)
        self.conv_out_2 = nn.Conv2d(depth_rate, in_channels, kernel_size=kernel_size, padding=(kernel_size - 1) // 2)

    def forward(self, x, x2):
        # x = [coarse_out, semantic_attention_weight] ---
        inp = x[0]
        inp2 = x2[0]
        semantic = x[1]
        semantic2 = x2[1]
        ...
        inp = self.conv_in(inp)
        inp2 = self.conv_in2(inp2)

        x_index = [[0 for _ in range(self.width)] for _ in range(2)]

        x_index[0][0] = self.rdb_in(inp)
        x_index[1][0] = self.rdb_in2(inp2)

        net_out = self.transform_feature_map_to_120(x_index[0][0])
        net_out = self.groupAttentionMul(net_out, semantic, group=30)
        net_out = self.transform_feature_map_to_depth_rate(net_out)
        x_index[0][0] = x_index[0][0] + net_out

        net_out_2 = self.transform_feature_map_to_120_2(x_index[1][0])
        net_out_2 = self.groupAttentionMul(net_out_2, semantic2, group=30)
        net_out_2 = self.transform_feature_map_to_depth_rate_2(net_out_2)
        x_index[1][0] = x_index[1][0] + net_out_2

        for j in range(1, self.width):
            x_index[0][j] = self.rdb_modules['{}_{}'.format(0, j-1)](x_index[0][j-1], x_index[1][j-1])
            x_index[1][j] = self.rdb_modules['{}_{}'.format(1, j-1)](x_index[1][j-1], x_index[0][j-1])
    
        out = self.rdb_out(x_index[0][5], x_index[1][5])
        out = F.relu(self.conv_out(out))
        out2 = self.rdb_out_2(x_index[1][5], x_index[0][5])
        out2 = F.relu(self.conv_out_2(out2))
...
class StereoImageNet_share(nn.Module):
    def __init__(self, need_semantic=True, multiscale=True, in_channels=3, width=6, depth_rate=16, kernel_size=3, num_dense_layer=4, growth_rate=16):
        super(StereoImageNet_share, self).__init__()
        self.width = width
        self.rdb_modules = nn.ModuleDict()
        self.depth_rate = depth_rate
        rdb_in_channels = depth_rate
        ...
        for j in range(self.width-1):
            self.rdb_modules.update({'{}_{}'.format(0,j): RDB_32(rdb_in_channels, num_dense_layer, growth_rate*2)})
    ...
    def forward(self, x, x2):
        ...
        inp = self.conv_in(inp)
        inp2 = self.conv_in(inp2)

        x_index = [[0 for _ in range(self.width)] for _ in range(2)]
        x_index[0][0] = self.rdb_in(inp)
        x_index[1][0] = self.rdb_in(inp2)
        ...
        net_out_2 = self.transform_feature_map_to_120(x_index[1][0])
        net_out_2 = self.groupAttentionMul(net_out_2, semantic2, group=30)
        net_out_2 = self.transform_feature_map_to_depth_rate(net_out_2)
        x_index[1][0] = x_index[1][0] + net_out_2

        for j in range(1, self.width):
            x_index[0][j] = self.rdb_modules['{}_{}'.format(0, j-1)](x_index[0][j-1], x_index[1][j-1])
            x_index[1][j] = self.rdb_modules['{}_{}'.format(0, j-1)](x_index[1][j-1], x_index[0][j-1])

        out = self.rdb_out(x_index[0][5], x_index[1][5])
        out = F.relu(self.conv_out(out))
        out2 = self.rdb_out(x_index[1][5], x_index[0][5])
        out2 = F.relu(self.conv_out(out2))
...
class Single_coarse_stereo_fine(nn.Module):
    def __init__(self, semantic_extract_model=None, in_channels=3, depth_rate=16, kernel_size=3, stride=2, height=1, width=6, num_dense_layer=4, growth_rate=16, share=False):
        super(Single_coarse_stereo_fine, self).__init__()
        self.coarse_model = GridDehazeNetSingle(height=height, multiscale=False, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
        self.level = 1
        self.scale = 1
        self.semantic_extract_model = semantic_extract_model
        if not share:
            print("fine net no share")
            self.fine_model = StereoImageNet_noshare(need_semantic=True, multiscale=True, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
        else:
            print("fine net share~")
            self.fine_model = StereoImageNet_share(need_semantic=True, multiscale=True, width=width, num_dense_layer=num_dense_layer, growth_rate=growth_rate)
        self.semantic_attention_block = AttentionBlock(init_channels=15, inc_rate=2, need_semantic=True)
        self.to_tensor = ToTensor()
    ...
    def forward(self, x, x2):
        ...
        coarse_out2 = self.coarse_model(x2)
        semantic_attention_weight2 = self.semantic_extract(coarse_out2)

        fine_out, fine_out2= self.fine_model([coarse_out, semantic_attention_weight], [coarse_out2, semantic_attention_weight2])

        return fine_out, fine_out2
