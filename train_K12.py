...
train_data_loader = DataLoader(K12_dataset(crop_size=crop_size, root=train_data_dir, single_stereo=single_stereo), batch_size=train_batch_size, shuffle=True, num_workers=0, drop_last=True)
...
while epoch < num_epochs:
    ...
    if epoch % 10 == 0:
        ...
        if fine_share:
            torch.save(state, './checkpoint/checkpoint_K12_epoch_{}'.format(epoch))
        else:
            torch.save(state, './checkpoint/checkpoint_K12_epoch_{}'.format(epoch))
